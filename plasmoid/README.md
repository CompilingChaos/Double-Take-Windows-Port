...
This widget is Linux/KDE Plasma only. The Windows port keeps the command-line
daemon and `doubletake-ctl` workflow, but Plasma widgets are not a Windows
feature.

## Prerequisites

- KDE Plasma 6 (Plasma 5 should also work with minor QML import adjustments)
- `doubletake` running (provides the local control endpoint)
