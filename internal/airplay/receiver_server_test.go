...
func TestMediaFirstAutoUsesHEVCWhenCommittedNTPLatencyIsSufficient(t *testing.T) {
	SetTargetLatency(0)
	t.Cleanup(func() { SetTargetLatency(0) })
	...
	session, err := client.SetupMirrorWithVideoCodecPreparation(ctx, StreamConfig{
		...
		if codec != VideoCodecHEVC {
			return fmt.Errorf("media-first codec = %s, want HEVC within the committed NTP lead", codec)
		}
		return nil
	})
	if err != nil {
	...
	if session.videoCodec != VideoCodecHEVC || session.timestampBias != ntpTargets.video {
		t.Fatalf("media-first session = codec %s lead %v, want HEVC/%v", session.videoCodec, session.timestampBias, ntpTargets.video)
	}
	if session.audioStream == nil || session.audioStream.latencySamples != samplesFor44k1(ntpTargets.audio) {
		t.Fatalf("media-first audio lead = %#v, want %d samples", session.audioStream, samplesFor44k1(ntpTargets.audio))
	}
}
...
func TestMediaFirstCalibratedAutoFallsBackBeforeLiveMeasurement(t *testing.T) {
	...
	ntpTargets := screenLatenciesForHint(connectionLatencyNormal).withMinimumVideoLead(ntpPlayoutLatencyFloor)
	if session.timestampBias != ntpTargets.video || session.audioStream == nil ||
		session.audioStream.latencySamples != samplesFor44k1(ntpTargets.audio) {
...
func TestMediaFirstForcedHEVCRejectsLateLiveLead(t *testing.T) {
	...
	_, err := client.SetupMirrorWithCalibratedVideoPreparation(ctx, StreamConfig{
		...
		return VideoPreparationResult{MinimumVideoLead: ntpPlayoutLatencyFloor + 175*time.Millisecond}, nil
	})
