...
// DefaultCredentialsPath returns the platform's default credentials path.
func DefaultCredentialsPath() string {
	if runtime.GOOS == "windows" {
		if appData := os.Getenv("APPDATA"); appData != "" {
			return filepath.Join(appData, "doubletake", "credentials.json")
		}
		home, _ := os.UserHomeDir()
		return filepath.Join(home, "AppData", "Roaming", "doubletake", "credentials.json")
	}
