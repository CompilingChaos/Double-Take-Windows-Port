...
func PairingAccessDeniedHint(info *ReceiverInfo) string {
	if info != nil && strings.Contains(strings.ToLower(info.Model), "mac") {
		return "pairing was rejected by the Mac. Enable AirPlay Receiver in System Settings > General > AirDrop & Handoff, and allow Anyone on the Same Network or Everyone."
	}
	return "pairing was rejected by the receiver. Check its AirPlay access settings and allow this sender before pairing again."
}
