...
func PairingAccessDeniedHint(info *ReceiverInfo) string {
	if info != nil && strings.Contains(strings.ToLower(info.Model), "mac") {
		return "pairing was rejected by the Mac. On the Mac, open System Settings > General > AirDrop & Handoff > AirPlay Receiver, then set Allow AirPlay for to Anyone on the Same Network or Everyone. If Require Password is enabled, run doubletake again with -pair -pin <password>."
	}
	return "pairing was rejected by the receiver. Check the receiver's AirPlay access settings and allow this sender before pairing again."
}
...
func (c *AirPlayClient) Connect(ctx context.Context) error {
	...
	for attempt := 1; attempt <= 3; attempt++ {
		...
		wait := time.Duration(attempt) * 750 * time.Millisecond
		dbg("[RTSP] connect %s attempt %d/3 failed: %v; retrying in %s", addr, attempt, err, wait)
